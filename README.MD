# WitchReader USB — Total/Double Commander file-system plugin (WFX)

Browse and manage the XTEink X3/X4 e-reader's SD card from **Total Commander** (Windows) or
...
2. Open any MSYS2 shell and install the toolchains once:
   ...
   ```
3. Right-click **`build.ps1`** → *Run with PowerShell* (or run it from a
   PowerShell prompt). It produces `dist\crosspoint-usb-wfx-windows.zip`
   containing both the 32-bit `.wfx` and 64-bit `.wfx64` binaries.
